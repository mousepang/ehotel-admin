  ...
  const title = (
    <span>
      <span style={{ fontSize: 14 }}>
        优惠卷管理 / &nbsp;
        <span style={{ fontSize: 13 }}>我的优惠卷</span>
      </span>
    </span>
  );
  const titleArr = [
        ...
        {titleArr.map((cardItem, titleIndex) => {
          return (
            <Card
              className="my-card"
              key={titleIndex}
              title={<span style={{ fontSize: 13 }}>{cardItem.title}</span>}
            >
                    ...
                    return (
                      <div className={cardItem.styles} key={index}>
                        ...
                        <span
                          ...
                          onClick={() => deleteCoupon(titleIndex, index)}
                        >
                          x
                        </span>
                        <Divider />
                        {item.couponType === 'fullCoupon' ? (
                          <span className="discount">
                            满{item.fullCoupon}&nbsp;减{item.subCoupon}
                          </span>
                        ) : (
                          <span className="discount">
                            {item.discountCoupon}折
                          </span>
                        )}
